...
const accent: Record<string, { border:string; hover:string; tag:string; arrow:string }> = {
          ...
          {blogPosts.map((post, i) => {
            ...
            return (
              <motion.article key={post.id}
                initial={{ opacity:0, y:20 }} animate={inView?{opacity:1,y:0}:{}} transition={{ duration:0.5, delay:0.08*i }}
                className={`glass-card rounded-xl p-4 sm:p-5 border ${a.border} ${a.hover} hover:shadow-lg transition-all duration-300 group flex flex-col`}>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {post.tags.map(tag => (
                    <span key={tag} className={`px-2 py-0.5 text-[10px] font-mono rounded border ${a.tag}`}>{tag}</span>
                  ))}
                </div>
                <h3 className="t-1 font-semibold text-sm sm:text-base leading-snug mb-2">{post.title}</h3>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 t-3 text-xs">
                    <Clock className="w-3.5 h-3.5" /><span>{post.readTime}</span>
                  </div>
                  <a href={post.href} className={`flex items-center gap-1 text-xs font-medium ${a.arrow} opacity-0 group-hover:opacity-100 transition-opacity`}>
