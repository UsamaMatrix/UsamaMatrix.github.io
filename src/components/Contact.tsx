...
const socialAccent: Record<string, { icon:string; bg:string; border:string; hover:string; glow:string }> = {
  cyan:   { icon:"text-cyan-400",   bg:"bg-cyan-400/10",   border:"border-cyan-400/20",   hover:"hover:border-cyan-400/50",   glow:"hover:shadow-cyan-500/12"   },
  blue:   { icon:"text-blue-400",   bg:"bg-blue-400/10",   border:"border-blue-400/20",   hover:"hover:border-blue-400/50",   glow:"hover:shadow-blue-500/12"   },
  red:    { icon:"text-red-400",    bg:"bg-red-400/10",    border:"border-red-400/20",    hover:"hover:border-red-400/50",    glow:"hover:shadow-red-500/12"    },
  purple: { icon:"text-purple-400", bg:"bg-purple-400/10", border:"border-purple-400/20", hover:"hover:border-purple-400/50", glow:"hover:shadow-purple-500/12" },
};
...
function Toast({ status, onClose }: { status:"success"|"error"; onClose:()=>void }) {
  ...
  return (
    <motion.div
      ...
      className={`fixed bottom-20 sm:bottom-6 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl border backdrop-blur-xl max-w-sm w-[calc(100%-2rem)] ${
        ok ? "bg-[#0d1f14] border-green-500/40 shadow-green-500/15" : "bg-[#1f0d0d] border-red-500/40 shadow-red-500/15"
      }`}
    >
      <motion.div
        initial={{ scale:0 }} animate={{ scale:1 }} transition={{ type:"spring", delay:0.1 }}
        className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${ok ? "bg-green-500/15" : "bg-red-500/15"}`}
      >
        {ok ? <CheckCircle className="w-5 h-5 text-green-400" /> : <AlertCircle className="w-5 h-5 text-red-400" />}
      </motion.div>
      <div className="flex-1 min-w-0">
        <p className={`font-semibold text-sm ${ok ? "text-green-300" : "text-red-300"}`}>
          {ok ? "Message sent!" : "Failed to send"}
        </p>
        <p className="text-xs mt-0.5 t-3">
          {ok ? "I'll get back to you as soon as possible." : "Something went wrong. Try emailing directly."}
        </p>
      </div>
      <button onClick={onClose} className="shrink-0 p-1 rounded-lg hover:bg-white/10 transition-colors t-3">
        <X className="w-4 h-4" />
      </button>
    </motion.div>
  );
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await emailjs.send(EJS_SERVICE, EJS_TEMPLATE, { name:form.name, email:form.email, service:form.service, message:form.message }, EJS_PUBLIC);
      setStatus("success"); setToast("success");
      setForm({ name:"", email:"", service:"", message:"" });
      setTimeout(() => setStatus("idle"), 4000);
    } catch (err: unknown) {
      const msg = err && typeof err === "object" && "text" in err ? String((err as {text:unknown}).text) : String(err);
      setError(`Send failed: ${msg}`); setStatus("error"); setToast("error");
      setTimeout(() => setStatus("idle"), 4000);
    }
  };
  ...
  const inputCls =
    ...
    "focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/15 focus:bg-[var(--bg-card)]";

  return (
    <>
      <section id="contact" className="py-16 sm:py-24 page-bg relative overflow-hidden">
        <div className="absolute inset-0 grid-bg opacity-25 pointer-events-none" />
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[500px] h-[200px] bg-cyan-500/4 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            {socials.map((s, i) => {
              ...
              return (
                <motion.a
                  ...
                  initial={{ opacity:0, y:20 }} animate={inView?{opacity:1,y:0}:{}} transition={{ duration:0.4, delay:0.08*i }}
                  whileHover={{ y:-4, scale:1.02 }}
                  whileTap={{ scale:0.97 }}
                  className={`glass-card rounded-xl p-4 sm:p-5 border ${a.border} ${a.hover} hover:shadow-xl ${a.glow} transition-all duration-300 text-center group`}
                >
                  <motion.div
                    className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl ${a.bg} flex items-center justify-center mx-auto mb-3`}
          ...
          {/* form card */}
          <motion.div
            initial={{ opacity:0, y:28 }} animate={inView?{opacity:1,y:0}:{}} transition={{ duration:0.6, delay:0.3 }}
          ...
          >
            ...
            <div className="px-6 sm:px-8 py-5 border-b" style={{ borderColor:"var(--border)" }}>
              <div className="flex items-center gap-3">
                <motion.div
                  className="w-9 h-9 rounded-lg bg-cyan-400/10 flex items-center justify-center"
                  animate={{ boxShadow:["0 0 0px rgba(6,182,212,0)","0 0 16px rgba(6,182,212,0.25)","0 0 0px rgba(6,182,212,0)"] }}
            ...
            <form onSubmit={handleSubmit} className="px-6 sm:px-8 py-6 sm:py-8 space-y-5">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-1">
                ...
                <motion.button
                  ...
                  className="flex items-center justify-center gap-2 px-6 py-2.5 bg-cyan-500 hover:bg-cyan-400 disabled:opacity-70 disabled:cursor-not-allowed text-black font-semibold rounded-lg text-sm transition-colors shrink-0 shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/35"
                >
                  {status === "sending" ? (
