...
const accent: Record<string, { icon:string; bg:string; tag:string; border:string; hover:string }> = {
  red:    { icon:"text-red-400",    bg:"bg-red-400/10",    tag:"bg-red-400/10 text-red-400 border-red-400/20",       border:"border-red-400/20",    hover:"hover:border-red-400/40"    },
  cyan:   { icon:"text-cyan-400",   bg:"bg-cyan-400/10",   tag:"bg-cyan-400/10 text-cyan-400 border-cyan-400/20",     border:"border-cyan-400/20",   hover:"hover:border-cyan-400/40"   },
  blue:   { icon:"text-blue-400",   bg:"bg-blue-400/10",   tag:"bg-blue-400/10 text-blue-400 border-blue-400/20",     border:"border-blue-400/20",   hover:"hover:border-blue-400/40"   },
  purple: { icon:"text-purple-400", bg:"bg-purple-400/10", tag:"bg-purple-400/10 text-purple-400 border-purple-400/20",border:"border-purple-400/20", hover:"hover:border-purple-400/40" },
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {expertiseCards.map((card, i) => {
            ...
            return (
              <motion.article key={card.id}
                initial={{ opacity:0, y:20 }} animate={inView?{opacity:1,y:0}:{}} transition={{ duration:0.5, delay:0.05*i }}
                className={`glass-card rounded-xl p-5 border ${a.border} ${a.hover} hover:shadow-lg transition-all duration-300`}>
